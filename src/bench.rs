...
#[bench]
fn bench_build_dfa_distance4_with_transpose(b: &mut Bencher) {
    let nfa = LevenshteinNFA::levenshtein(4, true);
    let parametric_dfa = ParametricDFA::from_nfa(&nfa);
    b.iter(|| {
        let _dfa = parametric_dfa.build_dfa("Levenshtein");
    });
...
#[bench]
fn bench_build_parametricdfa_damerau_perf_2(b: &mut Bencher) {
    let nfa = LevenshteinNFA::levenshtein(2, true);
    b.iter(|| {
        let parametric_dfa = ParametricDFA::from_nfa(&nfa);
        // Actually 7 is sufficient.
        ...
        // On the next extension our algorithm loses the information that
        // chi can only start by 11x.
        assert_eq!(parametric_dfa.num_states(), 68);
    });
}
